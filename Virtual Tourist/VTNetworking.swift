...
//  Virtual Tourist
//
//  Created by Lybron Sobers on 3/17/17.
...
extension URL {
  public static func from(scheme: String, host: String, path: String, parameters: [String:String]) -> URL {
    
    let string = scheme + host + path
...
extension URLSessionDataTask {
  public static func task(request: URLRequest, completion: @escaping (_ result: Data?, _ networkError: Error?) -> Void) -> URLSessionDataTask {
    ...
    let task = URLSession.shared.dataTask(with: request, completionHandler: { (data, response, error) in
      ...
      func sendError(_ error: VTNetworkError) {
        let userInfo = [NSLocalizedDescriptionKey : error.rawValue]
        completion(nil, NSError(domain: "VTURLSessionDataTask", code: 1, userInfo: userInfo))
      }
